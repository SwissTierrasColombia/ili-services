...
class TestIli2dbBaskets(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        cls.ili2db = Ili2DB()

        schema_name = 'ili2db_baskets'
    ...
    def test_ili2db(self):
        for db_engine, db in self.db_connections.items():
            print("\nINFO: Validating import schema, import data and export method in {} with multiples baskets...".format(db_engine))
            ...
            # Run import data
            xtf_path = get_test_path("xtf/test_baskets_field_data_capture_1_2.xtf")
            ...
            baskets = ['35f974c4-837b-4883-8a57-d3f0d5c888b4']
            configuration = self.ili2db.get_export_configuration(db, xtf_export_path,'',baskets)
            res_export, msg_export = self.ili2db.export(db, configuration)
            self.assertTrue(res_export, msg_export)

            # Update data
            xtf_update_data_path = get_test_path("xtf/test_update_data_field_data_capture_1_2.xtf")
            configuration = self.ili2db.get_update_configuration(db, xtf_update_data_path, '')
            res_update, msg_update = self.ili2db.update(db, configuration)
            self.assertTrue(res_update, msg_update)
